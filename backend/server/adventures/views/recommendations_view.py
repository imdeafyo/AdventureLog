...
from adventures.geocoding import _preferred_lang  # minimal add: reuse helper
...
class RecommendationsViewSet(viewsets.ViewSet):
    ...
    def parse_google_places(self, places, origin):
        ...
        for place in places:
            ...
            adventure = {
                ...
                "latitude": lat,
                "longitude": lon,
                "address": formatted_address,  # keep address under name
    ...
    def parse_overpass_response(self, data, request):
        ...
        # minimal add: honor UI language for OSM name fallback
        lang = _preferred_lang(getattr(request, "user", None))
        lang_key = f"name:{lang}" if lang else None
        base_lang = str(lang).split('-')[0] if lang else None
        ...
        for node in nodes:
            ...
            # Prefer localized, then official, then generic (minimal change)
            name = (
            ...
            # Flatten address (keep same behavior: show under name)
            ...
            adventure = {
                ...
                "latitude": lat,
                "longitude": lon,
                "address": formatted_address,  # still included
    ...
    def query_google_nearby(self, lat, lon, radius, category, request):
        ...
        # minimal add: honor language for Google too (safe no-op if key unset)
        lang = _preferred_lang(getattr(request, "user", None))
